...
Este componente va a tomar la información de cada una de las canciones y nos va a renderizar un elemento representando esta canción en una lista que vamos a generar a partir de la información que obtengamos de `getTopTracks` y `getRecentlyPlayed`. También vamos a crear un componente que nos generará estas listas con de elementos `TrackItem`. Para esto creamos un nuevo archivo llamado `TrackList.js` en la misma carpeta que el componente anterior:
