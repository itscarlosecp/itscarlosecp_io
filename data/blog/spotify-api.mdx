...
Este token es solamente temporal (lo identificaremos como `TEMPORAL_TOKEN`), pero lo utilizaremos para generar un token permanente. Para esto, primero tenemos que encriptar nuestras llaves de acceso, utilizando el codec base64.Podemos utilizar [base64encode](https://www.base64encode.org/) online para encriptar estos valores asegurémonos de seleccionar "encoding" en la parte superior). Debemos separar los valores de nuestros "Client ID" y nuestro "Client Secret" con dos puntos de por medio, de la siguiente forma:
